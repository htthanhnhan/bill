...
export default class Bill extends mixins(JhiDataUtils) {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.bills.push(res.data[i]);
              this.bills[i].JProduct = JSON.parse(this.bills[i].products);
            }
  ...
  public prepareRemove(instance: IBill): void {
  ...
  public removeBill(): void {
    this.billService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('billManagerApp.bill.deleted', { param: this.removeId });
