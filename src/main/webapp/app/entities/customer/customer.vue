<template>
  <div>
    <h2 id="page-heading" data-cy="CustomerHeading">
      <span v-text="$t('billManagerApp.customer.home.title')" id="customer-heading">Customers</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('billManagerApp.customer.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CustomerCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('billManagerApp.customer.home.createLabel')"> Create a new Customer </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && customers && customers.length === 0">
      <span v-text="$t('billManagerApp.customer.home.notFound')">No customers found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('fullName')">
              <span v-text="$t('billManagerApp.customer.fullName')">Full Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('email')">
              <span v-text="$t('billManagerApp.customer.email')">Email</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'email'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('address')">
              <span v-text="$t('billManagerApp.customer.address')">Address</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="billManagerApp.customer.delete.question" data-cy="customerDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-customer-heading" v-text="$t('billManagerApp.customer.delete.question', { id: removeId })">
